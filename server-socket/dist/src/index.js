...
app.post('/auth/login', (req, res) => {
	try {
		pool.query(
			...
			[req.query.valEmail, req.query.valPassword],
			(error, results) => {
...
app.post('/crmessage', (req, res) => {
	...
	jwt.verify(token, `secret123`, (err, decoded) => {
		if (err) {
			...
			pool.query(
				'SELECT room_id FROM room_users WHERE user_id = $1 OR user_id = $2 GROUP BY room_id HAVING COUNT(DISTINCT user_id) = 2',
				[decoded.id, req.body.id],
				(error, resultsRoom) => {
					const id = uuid()
					if (resultsRoom.rows.length) {
						...
						pool.query(
							'INSERT INTO rooms (id, room_type) VALUES($1, $2)',
							[id, 'two'],
							(error, results) => {
								if (error) throw error
								pool.query(
									'INSERT INTO room_users (id, room_id, user_id) VALUES($1, $2, $3)',
									[uuid(), id, decoded.id],
									(error, results) => {
										if (error) throw error
										pool.query(
	...
	try {
		if (
			...
			pool.query('SELECT * FROM users WHERE email = $1', [req.body.valEmail], (error, results) => {
				...
				if (!results.rows.length) {
					pool.query(
						'INSERT INTO users (id, user_name, user_fname, user_oname, email, password) VALUES ($1, $2, $3, $4, $5, $6)',
						[
							id,
							req.body.valName,
							req.body.valFname,
			...
			if (decoded.id) {
				pool.query(
					...
					(error, results) => {
						...
						pool.query(
							`SELECT room_users.id, room_users.room_id, room_users.user_id, users.* FROM room_users JOIN users ON room_users.user_id = users.id WHERE room_users.room_id IN (${newArr
								.map((item) => `'${item}'`)
								.join(',')}) AND user_id <> '${decoded.id}'`,
							(error, resultsRooms) => {
	...
	socket.on('pushRoomMessage', (data) => {
		try {
			...
			jwt.verify(token, `secret123`, (err, decoded) => {
				if (decoded.id) {
					pool.query(
						'INSERT INTO messages (room_id, user_id, message, data) VALUES($1, $2, $3, $4)',
						[data.room, decoded.id, data.message, data.data],
					)
				}

...
export const getComments = (req: Request, res: Response) => {
	try {
		pool.query(
			'SELECT comments.id, comments.description, users.name, users.fname, users.icon_url, comments.data FROM comments LEFT JOIN users ON comments.user_id = users.id WHERE comments.video_id = $1',
			[req.query.look],
